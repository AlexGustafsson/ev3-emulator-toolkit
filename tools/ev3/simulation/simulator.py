...
class Simulator:
    ...
    def __handle_motor_schedule(self, runtime: Runtime, block: Block) -> None:
        motor = block.fields["motor"].value
    ...
    def __handle_motor_pair_tank(self, runtime: Runtime, block: Block) -> None:
    ...
    def __handle_motor_pair_steer(self, runtime: Runtime, block: Block) -> None:
    ...
    def __handle_motor_pause_until_read(self, runtime: Runtime, block: Block) -> None:
        motor = block.fields["motor"].value
        logging.debug("Pausing until read motor={}".format(motor))

    def __handle_motor_stop(self, runtime: Runtime, block: Block) -> None:
        motors = block.fields["motors"].value
        logging.debug("Stopping motors={}".format(motors))

    def __handle_motor_reset(self, runtime: Runtime, block: Block) -> None:
        motors = block.fields["motors"].value
        logging.debug("Resetting motors={}".format(motors))

    def __handle_motor_stop_all(self, runtime: Runtime, block: Block) -> None:
        logging.debug("Stopping all motors")

    def __handle_motor_reset_all(self, runtime: Runtime, block: Block) -> None:
        logging.debug("Resetting all motors")

    def __handle_motor_clear_count(self, runtime: Runtime, block: Block) -> None:
        motor = block.fields["motor"].value
        logging.debug("Clearing count for motor={}".format(motor))

    def __handle_output_motor_set_brake_mode(self, runtime: Runtime, block: Block) -> None:
        motor = block.fields["motor"].value
        mode = block.values["brake"].shadow.fields["on"].value
        logging.debug("Setting motor brake mode motor={} mode={}".format(motor, mode))

    def __handle_device_pause(self, runtime: Runtime, block: Block) -> None:
        ms = self.__evaluate_value(block.values["pause"])
        logging.debug("Sleeping for {}ms".format(ms))

    def __handle_pxt_controls_for(self, runtime: Runtime, block: Block) -> None:
        pass

    def __handle_control_run_in_parallel(self, runtime: Runtime, block: Block) -> None:
        runtime.add_branch(block.statements["HANDLER"])

    def __handle_control_wait_us(self, runtime: Runtime, block: Block) -> None:
    ...
    def __handle_console_log_value(self, runtime: Runtime, block: Block) -> None:
        name = self.__evaluate_value(block.values["name"])
        value = self.__evaluate_value(block.values["value"])
        logging.debug("Logging value {}={}".format(name, value))
        print("{}={}".format(name, value))

    def __handle_button_wait_until(self, runtime: Runtime, block: Block) -> None:
        button = block.fields["button"].value
        event = block.fields["event"].value
        logging.debug("Waiting for event={} button={}".format(event, button))

    def __handle_colorpause_until_color_detected_detected(self, runtime: Runtime, block: Block) -> None:
    ...
    def __handle_color_pause_until_light_detected(self, runtime: Runtime, block: Block) -> None:
    ...
    def __handle_ultrasonic_wait(self, runtime: Runtime, block: Block) -> None:
        sensor = block.fields["this"].value
        event = block.fields["event"].value
        logging.debug("Waiting for sensor {} to detect event {}".format(sensor, event))

    def __handle_touch_wait_until(self, runtime: Runtime, block: Block) -> None:
        sensor = block.fields["this"].value
        event = block.fields["event"].value
        logging.debug("Waiting for sensor {} to detect event {}".format(sensor, event))

    def __handle_screen_show_image(self, runtime: Runtime, block: Block) -> None:
        image = self.__evaluate_value(block.values["image"])
        logging.debug("Showing image {}".format(image))

    def __handle_screen_print(self, runtime: Runtime, block: Block) -> None:
    ...
    def __handle_screen_show_number(self, runtime: Runtime, block: Block) -> None:
        name = self.__evaluate_value(block.values["name"])
        line = self.__evaluate_value(block.values["line"])
        logging.debug("Printing name={} on line={}".format(name, line))

    def __handle_screen_show_value(self, runtime: Runtime, block: Block) -> None:
    ...
    def __handle_brick_show_ports(self, runtime: Runtime, block: Block) -> None:
        logging.debug("Show brick ports")

    def __handle_screen_clear_screen(self, runtime: Runtime, block: Block) -> None:
